...
mappages(pde_t *pgdir, void *la, uint size, uint pa, int perm)
{
  ...
  a = PGROUNDDOWN(la);//extracting first 20 pages setting the last 12 no need to modify
...
// of it for a child.
//TODO: map parent's physical pages into the child, instead of allocating new pages. Clear PTE_W in the PTEs of both child
//and parent for pages that have PTE_W set
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  if((d = setupkvm()) == 0)//look at setupkvm creates mapping based on addresses and base directory
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)//walkpgdir indexes in the page table and returns the final entry of that pagetable
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    flags = PTE_FLAGS(*pte);//need to set PTE flag to read only
    if((mem = kalloc()) == 0)//need to remove kalloc call
      goto bad;
    
    memmove(mem, (char*)pa, PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, PADDR(mem), flags) < 0)//need to pass p instead of mem when mapping 
      goto bad;
  }
  return d;
...
pde_t* cowuvm(pde_t *pgdir, uint sz){
	...
	for(i = 0; i < sz; i += PGSIZE){
		...
		*pte = (~ PTE_W) & *pte; //setting flag to read only parent page
		flags = PTE_FLAGS(*pte);
		pa = PTE_ADDR(*pte);
